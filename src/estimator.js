const inputData = {
  ...
  const covid19ImpactEstimator = (data) => {
    ...
    const outputData = {
      ...
      impact: {
        ...
        infectionsAsByRequestedTime: () => {
            ...
            if (data.periodType == 'days' ){
                let power = Math.floor(data.timeToElapse/3);
                return outputData.impact.currentlyInfected * Math.pow(2,power);
            }else if( data.periodType == 'weeks' ) {
                let days = (data.timeToElapse * 7);
                let power = Math.floor(days/3);
                return outputData.impact.currentlyInfected * Math.pow(2,power);  
            }else if ( data.periodType == 'months') {
                let days = ( data.timeToElapse * 30 );
                let power = Math.floor(days/3);
                return outputData.impact.currentlyInfected * Math.pow(2,power);
            }else{
      ...
      // tests for severe
    //   console.log(outputData.severeCasesByRequestedTime.hospitalBedsByRequestedTimeForSevereImpact())
    //   console.log(outputData.casesForICUByRequestedTime.severePositiveForICUAsPerSevereImpact())
